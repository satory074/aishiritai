...
Inside era MDX, `<Term>`, `<PersonCard>`, `<Milestone>`, `<Figure>`, and `<Prereq>` are used **without imports** — they are injected by `eras/[id].astro` via `<Content components={{ Term, PersonCard, Milestone, Figure, Prereq }} />`. Per-chapter SVG figure components (`src/components/figures/*.astro`) ARE imported at the top of each MDX file and passed into `<Figure>`. If you add a new MDX-level shared component, you must register it in that `components` object or MDX won't resolve it.
